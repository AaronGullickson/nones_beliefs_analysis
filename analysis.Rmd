...
- Belief in afterlife (2 categories, yes/no)
...
Note that because the afterlife question is binary, the scale I create for this variable is just equivalent to the proportion who said yes. 

```{r create_simple_scale, echo=FALSE}
gss$god_scale <- (as.numeric(gss$god)-1)/5
gss$postlife_scale <- as.numeric(gss$postlife=="yes")
...
Belief in afterlife is a simpler yes/no question. I use binary logit models to predict respondents who said yes. I use the same model format as for the linear probability model estimated in the first section. 


```{r postlife_models, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, results='asis'}
model.afterlife.year <- glm((postlife=="yes")~year.centered, data=gss, weights=wtssall, 
                            family=binomial(logit))
model.afterlife.nones <- update(model.afterlife.year, .~.+nones)
model.afterlife.interact <- update(model.afterlife.year, .~.+nones*year.centered)
model.afterlife.spline <- update(model.afterlife.interact, .~.+nones*year.spline)
models <- list(model.afterlife.year, model.afterlife.nones,
               model.afterlife.interact, model.afterlife.spline)
...
stargazer(models,
          ...
          dep.var.labels.include = FALSE,
          dep.var.caption = "",
          title="Binary logit models predicting belief in the after-life",
          add.lines=list(bic),
          keep.stat=c("n","bic"))
```

These results here are generally consistent with the LPM results, although the results for the main effect of year spline and the linear year trend before 2000 for the nones are not significant. So it is not clear that the nones were increasing belief in the afterlife faster than the affiliated before 2000. The difference here is that the affiliated were closer to 1, so the increases by the nones should have been larger in terms of proportion to have the same effect on the odds. However, either way, the spline interaction clearly shows the reversal after 2000. I can also plot this fit in a figure, where is also very easy to see the change after 2000. 

```{r postlife_figure, echo=FALSE, error=FALSE, message=FALSE}
lodds <- predict(model.afterlife.spline, data.predict)
predicted.spline <- cbind(data.predict[,c("year","nones")], 
                   Percent=100*exp(lodds)/(1+exp(lodds)))



trend <- na.omit(as.data.frame.table(prop.table(xtabs(wtssall~postlife+year+nones,
                                                     data=gss),c(2,3))))
trend <- subset(trend, postlife=="yes", select=c("year","nones","Freq"))
colnames(trend) <- colnames(predicted.spline) <- c("Year","Affiliation","Percent")
trend$Year <- as.numeric(as.character(trend$Year))
trend$Percent <- 100*trend$Percent
...
ggplot(trend, aes(x=Year, y=Percent, group=Affiliation, shape=Affiliation))+
  ...
  geom_smooth(span=0.75, se=FALSE, linetype=1, aes(color=Affiliation))+
  geom_line(data=predicted.spline, colour="grey40", size=1)+  
  geom_vline(aes(xintercept=2000), linetype=3)+
  theme_bw()+
  theme(legend.position = "bottom")+
  scale_color_brewer(palette = "Dark2")
